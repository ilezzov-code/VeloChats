package ru.ilezzov.plugin.velocity;
...
import ru.ilezzov.plugin.velocity.command.MyCommandManager;
import ru.ilezzov.plugin.velocity.file.FileManager;
import ru.ilezzov.plugin.velocity.file.config.Config;
import ru.ilezzov.plugin.velocity.file.message.Messages;
import ru.ilezzov.plugin.velocity.manager.FastReplyManager;
import ru.ilezzov.plugin.velocity.manager.PlaceholderManager;
import ru.ilezzov.plugin.velocity.manager.PlaySoundManager;
import ru.ilezzov.plugin.velocity.manager.SpyManager;
import ru.ilezzov.plugin.velocity.model.Response;
import ru.ilezzov.plugin.velocity.properties.MyProperties;
import ru.ilezzov.plugin.velocity.version.VersionDate;
import ru.ilezzov.plugin.velocity.version.VersionManager;
import ru.ilezzov.plugin.velocity.version.VersionType;

import java.nio.file.Path;
import java.util.function.Consumer;

import static ru.ilezzov.plugin.velocity.logging.Lang.*;
import static ru.ilezzov.plugin.velocity.manager.PlaySoundManager.SOUND_CHANNEL;
import static ru.ilezzov.plugin.velocity.message.PluginMessage.*;
import static ru.ilezzov.plugin.velocity.utils.LegacySerialize.serializeToANSI;

@Plugin(id = "velochats", name = "VeloChats", version = ru.ilezzov.plugin.BuildConstants.VERSION, description = ru.ilezzov.plugin.BuildConstants.DESCRIPTION, url = "https://t.me/ilezovofficial", authors = {"ILeZzoV"})
public class Main {
    ...
    private static MyCommandManager commandManager;
    private static FastReplyManager replyManager;
    private static SpyManager spyManager;
    private static PlaySoundManager playSoundManager;
    ...
    public void onProxyInitialization(ProxyInitializeEvent event) {
        ...
        replyManager = new FastReplyManager();
        spyManager = new SpyManager(this.proxyServer);

        commandManager = new MyCommandManager(proxyServer);
        commandManager.loadCommands();

        playSoundManager = new PlaySoundManager(proxyServer);
        proxyServer.getChannelRegistrar().register(SOUND_CHANNEL);
    ...
    public static FastReplyManager getReplyManager() {
        return replyManager;
    }
    ...
    public static MyCommandManager getCommandManager() {
        return commandManager;
    }

    public static PlaySoundManager getPlaySoundManager() {
